...
class SegmentationModel:
            ...
            if os.path.isdir(os.path.join(self.PATH_LABELED, folder)):
                ...
                for f in os.listdir(os.path.join(self.PATH_LABELED, folder)):
                    if f.endswith(".png"):
                        ...
                        img = load_img(os.path.join(self.PATH_LABELED, folder, f))
                        m.append(img)
    ...
    def get_feature_vector_of_window(
        ...
        response_2 = self.multiscale_statistics.process(window).astype(np.uint8)
        num_pixels = response_1.shape[0] * response_1.shape[1]
        response_img = np.dstack((response_1, response_2))

        if ravel:
            return (
                response_img.reshape(
                    (window.shape[0] * window.shape[1], response_img.shape[-1])
                ),
                num_pixels,
            )
        else:
    ...
    def train(
        ...
        Args:
            ...
            use_minibatch (bool, optional): Specifies whether to use minibatch K-Means algorithm.
                                            Defaults to False.
            minibatch_size (int, optional): Batch size to use in minibatch K-Means.
        ...
        for label in self.feature_vectors_of_label:
            ...
            if use_minibatch:
                self.textons[label] = MiniBatchKMeans(
                    n_clusters=K, batch_size=minibatch_size
                ).fit(self.feature_vectors_of_label[label])
            else:
